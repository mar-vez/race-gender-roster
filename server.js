...
async function isImbalance(input) {

  const { stdout, stderr } = await exec(`python3 ./RJA1.py '"${escapeStringRegexp(input)}"'`);
  if (stderr) {
    return stderr;
  }
  console.log(stdout);
...
io.on('connection', (socket) => {
  ...
  if (!cache[user.id]) {
    ...
    const message = `Hello, I will analyze all of your messages for roster analysis.`;
  ...
  */
  socket.on('request', async payload => {
    ...
    const result = await isImbalance(payload.value);
